...
void Producer(int bufSize, int itemCnt, int randSeed) {
  ...
  // "in" and "out" Use the provided function WriteAtBufIndex() to write into
  // the bounded buffer Use the provided function GetRand() to generate a random
  // number in the specified range
  // **Extremely Important: Remember to set the value of any shared variable you
  // change locally Use the following print statement to report the production
  // of an item: printf("Producing Item %d with value %d at Index %d\n", i, val,
  // in); where i is the item number, val is the item value, in is its index in
  // the bounded buffer

  // create itemCnt number of random numbers to write into the buffer index
